...
def add_default():
    idx = 0
    fpath = 'all_features'
    for p_in in ['donations.dat', 'essays.dat', 'projects.dat', 'resources.dat']: 
        with open('%s/%s'%(fpath, p_in)) as fin:
            fin.readline()
            for line in fin:
                row = line.strip().split(',')
                pid = row[0]
                if p_in == 'projects.dat':
                    feat = row[1:-1]
                else:
                    feat = row[1:]
                n_feat = len(feat)
                F_len = [(idx+i, float(v)) for i, v in enumerate(feat)]
                add_feats(pid, F_len)
        idx += n_feat
